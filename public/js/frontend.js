        		...
        		$.ajax({
        			...
        			success: function (data, textStatus, jqXHR) {
                        ...
                        if (data.status == 1) {
							...
							var score_width=data.data.score*15;
							$comment_tpl.find(".scorebox span").css("width",score_width);
                    		var $comment_postbox=form.find(".comment-postbox");
                    		var comment_content=$comment_postbox.val();
							var anony = data.data.anony;
							if(anony == 0){
								$comment_tpl.find('.comment-content a').text("匿名用户");
								$comment_tpl.find('.avatar').remove();
								$comment_tpl.attr("data-username","匿名用户");
							}
