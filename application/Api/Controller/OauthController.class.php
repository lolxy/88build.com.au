	...
	private function _bang_handle($user_info, $type, $token){
		...
		if($find_oauth_user){
			...
			if($find_oauth_user['uid']==$current_uid){
				$this->error("您之前已经绑定过此账号！",U('portal/index/index'));exit;
			}else{
				$this->error("该帐号已被本站其他账号绑定！",U('portal/index/index'));exit;
			}
		...
		if($need_bang){
			...
			if($current_uid){
				...
				if($new_oauth_user_id){
					$this->success("绑定成功！",U('portal/index/index'));
				}else{
					$this->error("绑定失败！",U('portal/index/index'));
				}
			}else{
				$this->error("绑定失败！",U('portal/index/index'));
			}
