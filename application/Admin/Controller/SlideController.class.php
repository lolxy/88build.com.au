...
class SlideController extends AdminbaseController{
	...
	public function index(){
		...
		if($user_type == 1){
			...
			$slides=$this->slide_model->where($where)->where(array("user_id"=>sp_get_current_admin_id()))->order("listorder ASC")->select();
		}
	...
	public function add(){
		...
		if($user_type == 1){
			...
			$categorys=$this->slidecat_model->field("cid,cat_name")->where("cat_status!=0 and cid=2")->find();
		}
		$this->assign("user_type",$user_type);
		$this->assign("categorys",$categorys);
		$this->display();
