	...
	public function post(){
		if (IS_POST){
			...
			if ($data!==false){
				...
				$this->comments_model->score=$_POST['score'];
				...
				if ($result!==false){
					...
					if($_POST['comment_type']==0){
						$post_table_model->where(array('id'=>$_POST['post_id']))->setInc('comment_count');
					}else{
						$post_table_model->where(array('id'=>$_POST['post_id']))->setInc('post_comment_count');
					}
							
					if($_POST['score'] == 5){
					$post_table_model->create(array("user_love"=>array("exp","user_love+1")));
					}
					...
					$this->ajaxReturn(sp_ajax_return(array("id"=>$result,"score"=>$_POST['score'],"anony"=>$_POST['type']),"评论成功！",1));
				} else {
					$this->error("评论失败！");
				}
			} else {
