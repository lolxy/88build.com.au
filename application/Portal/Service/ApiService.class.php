...
class ApiService {
    ...
    public static function posts($tag,$where=array(),$pagesize=0,$pagetpl=''){
    	...
    	if (isset($tag['ids'])) {
    		...
    		$where['term_relationships.posts_id'] = array('in',$tag['ids']);
    	}
    	...
    	$join = '__POSTS__ as posts on term_relationships.posts_id = posts.id';
    ...
    public static function postsPagedByTermId($term_id,$tag,$pagesize=20,$pagetpl=''){
        ...
        if(!empty($term_ids)){
            $where['term_relationships.term_id']=array('in',$term_ids);
			//筛选去除重复的文章
			$post_ids=M("TermRelationships")->distinct(true)->field("posts_id")->where(array("term_id"=>array('in',$term_ids)))->order('posts_id asc')->getField('posts_id',true);
			$where['term_relationships.posts_id']=array('in',$post_ids);
        }
    ...
    //获取用户列表（根据分类查询）
    public static function users($tag,$where=array(),$pagesize=0,$pagetpl=''){
    	$where=is_array($where)?$where:array();
    	$tag=sp_param_lable($tag);
    	...
    	$join = '__USERS__ as users on term_relationships.object_id = users.id';
    	
    	$term_relationships_model= M("TermRelationships");
    	$content=array();
    
    	if (empty($pagesize)) {
    	    $users=$term_relationships_model
    	    ->alias("term_relationships")
    	    ->join($join)
            ->distinct(true)
    	    ->field($field)
    ...
    public static function usersPaged($tag,$pagesize=20,$pagetpl=''){
        return self::users($tag,array(),$pagesize,$pagetpl);
    }

    public static function usersByTermId($term_id,$tag,$where=array()){
        $term_id=intval($term_id);
        ...
        $content=self::users($tag,$where);
        
        return $content['users'];
    }

    public static function usersPagedByTermId($term_id,$tag,$pagesize=20,$pagetpl=''){
        $term_id=intval($term_id);
        $term_ids=array();
        ...
        $content=self::users($tag,$where,$pagesize,$pagetpl);
