...
class AdminJingyanController extends AdminbaseController {
	...
	public function index(){
		...
		$user_type=$this->user_model->where("id=$user_id")->getField("user_type");
		 if($user_type == 1){
		 	...
		 	$this->_lists(array("post_status"=>array('neq',3),"post_author"=>array('eq',sp_get_current_admin_id())));
		 }
		$this->_getTree();
		$this->display();
	}
		...
		if (IS_POST) {
			...
			if ($result) {
				foreach ($_POST['term'] as $mterm_id){
					$this->term_relationships_model->add(array("term_id"=>intval($mterm_id),"posts_id"=>$result));
				}
	...
	private function _lists($where=array()){
		...
		$where['post_type']=array(array('eq',3),array('exp','IS NULL'),'OR');
		...
		if(!empty($term_id)){
		    $this->posts_model->join("__TERM_RELATIONSHIPS__ b ON a.id = b.posts_id");
		}
		...
		if(IS_POST){
			if(isset($_GET['ids']) && $_GET['old_term_id'] && isset($_POST['term_id'])){
			    ...
			    if($old_term_id!=$term_id){
			        ...
			        foreach ($ids as $id){
			            $this->term_relationships_model->where(array('posts_id'=>$id,'term_id'=>$old_term_id))->delete();
			            $find_relation_count=$this->term_relationships_model->where(array('posts_id'=>$id,'term_id'=>$term_id))->count();
			            if($find_relation_count==0){
			                $this->term_relationships_model->add(array('posts_id'=>$id,'term_id'=>$term_id));
			            }
			        }
	        ...
	        if(isset($_GET['ids']) && isset($_POST['term_id'])){
	            ...
	            foreach ($ids as $id){
	                ...
	                if($find_post){
	                    ...
	                    if($post_id>0){
	                        array_push($data, array('posts_id'=>$post_id,'term_id'=>$term_id));
	                    }
	                }
	            }
	...
	public function clean(){
		if(isset($_POST['ids'])){
			...
			$this->term_relationships_model->where(array('posts_id'=>array('in',$ids)))->delete();
			...
			if(isset($_GET['id'])){
				...
				$this->term_relationships_model->where(array('posts_id'=>$id))->delete();
