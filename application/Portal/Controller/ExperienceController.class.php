...
class ExperienceController extends HomebaseController {
    ...
    public function index() {
    	...
    	->where(array('b.id'=>array('gt',$article_id),"post_date"=>array("egt",$article_date),"a.status"=>1,'a.term_id'=>$term_id,'post_type'=>3,'post_status'=>1))
    	...
    	->alias("a")
    	->join($join)->join($join2)
    	->where(array('b.id'=>array('lt',$article_id),"post_date"=>array("elt",$article_date),"a.status"=>1,'a.term_id'=>$term_id,'post_type'=>3,'post_status'=>1))
    ...
    public function add_post(){
        if(IS_POST){
            ...
            if ($posts_model->field('post_type,post_date,post_author,post_content,post_title,post_modified,smeta')->create($article)!==false) {
                ...
                if ($result) {
                    $result=$term_relationships_model->add(array("term_id"=>intval($_POST['term']),"posts_id"=>$result));
                    if($result){
                        $this->success("经验添加成功！");
                    }else{
