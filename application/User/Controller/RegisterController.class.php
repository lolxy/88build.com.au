...
class RegisterController extends HomebaseController {
	...
	public function perfect(){
	   ...
	   if(!empty($userid)){
	   		$username = $usermodel->where(array("id"=>$userid))->getField('user_login');
			if(empty($username)){
		   ...
		   $this->error("您还没有登录，请登录后再完善资料！",U("user/login/index"));
		   }
	    ...
	    if($result || $uc_checkemail<0 || $uc_checkusername<0){
	        ...
	        if($uc_register){
	            ...
	            $data=array(
	                ...
	                "user_type"=>$usertype,//会员
	            );
	            $rst = $users_model->where(array("id"=>$userid))->save($data);
	            if($rst){
	                //注册成功页面跳转
	                $data['id']=$rst;
	                session('user',$data);
	                $role_user_model->add(array("role_id"=>$usertype,"user_id"=>$userid));	
	                //发送激活邮件
	                if($need_email_active){
	                    $this->_send_to_active();
	                    session('user',null);
	                    $this->success("资料完善成功，激活后才能使用！",U("user/login/index"));
	                }else {
	                    $this->success("资料完善成功！",__ROOT__."/");
	                }
	                	
	            }else{
	                $this->error("资料完善失败！",U("user/register/perfect"));
	            }
			...
			$this->success("您还没有登录，请登录后再完善资料！",U("user/login/index"));
			}
	}
	...
	private function _do_mobile_register(){
	    ...
	    if($result){
	        ...
	        $data['id']=$result;
	        session('user',$data);
			$role_user_model->add(array("role_id"=>$usertype,"user_id"=>$result));
	    ...
	    if($result || $uc_checkemail<0 || $uc_checkusername<0){
	        ...
	        if($uc_register){
	            ...
	            if($rst){
	                ...
	                $data['id']=$rst;
	                session('user',$data);
	                $role_user_model->add(array("role_id"=>$usertype,"user_id"=>$rst));	
	                //发送激活邮件
	                if($need_email_active){
	                    $this->_send_to_active();
	                    session('user',null);
	                    $this->success("注册成功，激活后才能使用，请打开邮箱，查收邮件，激活账户！",U("user/login/index"));
	                }else {
	                    $this->success("注册成功！",__ROOT__."/");
	                }
	...
	public function active(){
		...
		if($find_user){
			...
			if($result){
				...
				$this->success("用户激活成功，正在登录中...",U("user/center/index"));
			}else{
